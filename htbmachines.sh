...
function helpPanel(){
  ...
  echo -e "\t${purpleColour}m)${endColour}${grayColour} Buscar por una máquina${endColour}"
...
function updatefiles(){
  ...
  if [ ! -f bundle.js ]; then
    ...
    if [ "$md5_temp_value" == "$md5_original_value" ]; then
      echo -e "\n${yellowColour}[+]${endColour}${grayColour} No se han detectado actualizaciones, todo actualizado al día${endColour}"
      rm bundle_temp.js
    else
      echo -e "\n${yellowColour}[+]${endColour}${grayColour} Actualizaciones disponibles${endColour}"
      sleep 1
...
function getYoutubeLink(){
  ...
  if [ "$youtubeLink" ]; then
    echo -e "\n${yellowColour}[!]${endColour}${grayColour} El tutorial para esta máquina está en el siguiente link:${endColour}${blueColour} $youtubeLink${endColour}\n"
  else
    echo -e "\n${redColour}[!] La máquina proporcionada no existe${endColour}\n"
  fi
...
function getOSDifficultyMachines(){
 ...
 if [ "$check_results" ]; then
   echo -e "\n${yellowColour}[+]${endColour}${grayColour} Listando máquinas de dificultad ${endColour}${blueColour} $difficulty${endColour}${grayColoyr} que tengan sistema operativo${endColour}${purpleColour} $os${endColour}${yellowColour}:${endColour}\n"
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} A continuación se representan las máquinas donde se ve Skill${endColour}${blueColour} $skill${endColour}${grayColour}:${endColour}\n"
